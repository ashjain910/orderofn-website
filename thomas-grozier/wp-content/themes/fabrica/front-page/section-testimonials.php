<div class="front_page_section front_page_section_testimonials<?php
	$fabrica_scheme = fabrica_get_theme_option( 'front_page_testimonials_scheme' );
	if ( ! empty( $fabrica_scheme ) && ! fabrica_is_inherit( $fabrica_scheme ) ) {
		echo ' scheme_' . esc_attr( $fabrica_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( fabrica_get_theme_option( 'front_page_testimonials_paddings' ) );
	if ( fabrica_get_theme_option( 'front_page_testimonials_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$fabrica_css      = '';
		$fabrica_bg_image = fabrica_get_theme_option( 'front_page_testimonials_bg_image' );
...
<?php
	// Add anchor
	$fabrica_anchor_icon = fabrica_get_theme_option( 'front_page_testimonials_anchor_icon' );
	$fabrica_anchor_text = fabrica_get_theme_option( 'front_page_testimonials_anchor_text' );
...
?>
	...
	<?php
	if ( fabrica_get_theme_option( 'front_page_testimonials_fullheight' ) ) {
	...
	?>
			...
			<?php
			$fabrica_css      = '';
			$fabrica_bg_mask  = fabrica_get_theme_option( 'front_page_testimonials_bg_mask' );
			$fabrica_bg_color_type = fabrica_get_theme_option( 'front_page_testimonials_bg_color_type' );
			if ( 'custom' == $fabrica_bg_color_type ) {
				$fabrica_bg_color = fabrica_get_theme_option( 'front_page_testimonials_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_testimonials_content_wrap content_wrap">
			<?php
			// Caption
			$fabrica_caption = fabrica_get_theme_option( 'front_page_testimonials_caption' );
			if ( ! empty( $fabrica_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_testimonials_caption front_page_block_<?php echo ! empty( $fabrica_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $fabrica_caption, 'fabrica_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$fabrica_description = fabrica_get_theme_option( 'front_page_testimonials_description' );
			if ( ! empty( $fabrica_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_testimonials_description front_page_block_<?php echo ! empty( $fabrica_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $fabrica_description ), 'fabrica_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_testimonials_output">
				...
				if ( is_active_sidebar( 'front_page_testimonials_widgets' ) ) {
					...
					if ( ! fabrica_exists_trx_addons() ) {
						...
						fabrica_customizer_need_widgets_message( 'front_page_testimonials_caption', 'ThemeREX Addons - Testimonials' );
					}
				}
				?>
			</div>
