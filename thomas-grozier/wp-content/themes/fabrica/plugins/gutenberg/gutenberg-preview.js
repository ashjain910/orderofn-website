...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			fabrica_create_observer( 'fabrica-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				fabrica_gutenberg_editor_init();
			} );
		} else {
		 	fabrica_gutenberg_editor_init();
		}
	...
	function fabrica_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'fabrica_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'fabrica_inited' ) ) {
	...
	// Init on page load
	function fabrica_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = fabrica_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + FABRICA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + FABRICA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in FABRICA_STORAGE['override_classes'] ) {
				$obj.addClass( FABRICA_STORAGE['override_classes'][i].replace( '%s', FABRICA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( fabrica_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && FABRICA_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'fabrica_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			fabrica_remove_observer( 'fabrica-check-editor-styles-wrapper' );
			fabrica_create_observer( 'fabrica-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'fabrica_inited' );
	}
} );
