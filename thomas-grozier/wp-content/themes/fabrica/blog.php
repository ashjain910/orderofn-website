...
if ( function_exists( 'fabrica_elementor_is_preview' ) && fabrica_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			fabrica_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	fabrica_new_main_query(
		array(
			'post_type'      => fabrica_get_theme_option( 'post_type' ),
			'category'       => fabrica_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'fabrica_filter_get_template_part', fabrica_blog_archive_get_template() ) );
}
