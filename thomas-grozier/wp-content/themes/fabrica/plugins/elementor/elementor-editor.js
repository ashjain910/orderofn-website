...
jQuery( document ).ready(
	function() {
		...
		fabrica_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="fabrica_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.fabrica_post_editor' );
						if ( link.length > 0 ) {
							fabrica_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.fabrica_post_editor', function(e) {
						fabrica_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('fabrica_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function fabrica_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
