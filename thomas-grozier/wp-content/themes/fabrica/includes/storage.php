...
if ( ! function_exists( 'fabrica_storage_get' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_get( $var_name, $default = '' ) {
		global $FABRICA_STORAGE;
		return isset( $FABRICA_STORAGE[ $var_name ] ) ? $FABRICA_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'fabrica_storage_set' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_set( $var_name, $value ) {
		global $FABRICA_STORAGE;
		$FABRICA_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'fabrica_storage_empty' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $FABRICA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $FABRICA_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $FABRICA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_isset' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $FABRICA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $FABRICA_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $FABRICA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_unset' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $FABRICA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $FABRICA_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $FABRICA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_inc' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_inc( $var_name, $value = 1 ) {
		global $FABRICA_STORAGE;
		if ( empty( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = 0;
		}
		$FABRICA_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'fabrica_storage_concat' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_concat( $var_name, $value ) {
		global $FABRICA_STORAGE;
		if ( empty( $FABRICA_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'fabrica_storage_get_array' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $FABRICA_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $FABRICA_STORAGE[ $var_name ][ $key ] ) ? $FABRICA_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'fabrica_storage_set_array' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_set_array( $var_name, $key, $value ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FABRICA_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'fabrica_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function fabrica_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $FABRICA_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRICA_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$FABRICA_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$FABRICA_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'fabrica_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_merge_array( $var_name, $key, $value ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$FABRICA_STORAGE[ $var_name ] = array_merge( $FABRICA_STORAGE[ $var_name ], $value );
		} else {
			$FABRICA_STORAGE[ $var_name ][ $key ] = array_merge( $FABRICA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fabrica_array_insert_after( $FABRICA_STORAGE[ $var_name ], $after, $key );
		} else {
			fabrica_array_insert_after( $FABRICA_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function fabrica_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			fabrica_array_insert_before( $FABRICA_STORAGE[ $var_name ], $before, $key );
		} else {
			fabrica_array_insert_before( $FABRICA_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_push_array' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_push_array( $var_name, $key, $value ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $FABRICA_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $FABRICA_STORAGE[ $var_name ][ $key ] ) ) {
				$FABRICA_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $FABRICA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'fabrica_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $FABRICA_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $FABRICA_STORAGE[ $var_name ] ) && is_array( $FABRICA_STORAGE[ $var_name ] ) && count( $FABRICA_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $FABRICA_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $FABRICA_STORAGE[ $var_name ][ $key ] ) && is_array( $FABRICA_STORAGE[ $var_name ][ $key ] ) && count( $FABRICA_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $FABRICA_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'fabrica_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function fabrica_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FABRICA_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRICA_STORAGE[ $var_name ][ $key ] = 0;
		}
		$FABRICA_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'fabrica_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function fabrica_storage_concat_array( $var_name, $key, $value ) {
		global $FABRICA_STORAGE;
		if ( ! isset( $FABRICA_STORAGE[ $var_name ] ) ) {
			$FABRICA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $FABRICA_STORAGE[ $var_name ][ $key ] ) ) {
			$FABRICA_STORAGE[ $var_name ][ $key ] = '';
		}
		$FABRICA_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'fabrica_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $FABRICA_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FABRICA_STORAGE[ $var_name ] ) ? $FABRICA_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $FABRICA_STORAGE[ $var_name ] ) ? $FABRICA_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'fabrica_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function fabrica_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $FABRICA_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $FABRICA_STORAGE[ $var_name ]->$prop ) ? $FABRICA_STORAGE[ $var_name ]->$prop : $default;
	}
}
