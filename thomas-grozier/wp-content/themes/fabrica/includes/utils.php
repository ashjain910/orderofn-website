			...
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							...
							apply_filters( 'fabrica_filter_cookie_options', array(
								...
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//fabrica_get_domain_from_url( get_home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//fabrica_get_domain_from_url( get_home_url() ),
							$secure,
							$httponly
						);
			}
...
if ( ! function_exists( 'fabrica_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'fabrica_init_filesystem', 0 );
	/**
	 ...
	 */
	function fabrica_init_filesystem( $force = false ) {
			...
			if ( function_exists( 'trx_addons_fpc' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
						...
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'fabrica' ), $file ) );
					}
				}
			}
		}
	...
	function fabrica_fgc( $file ) {
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				global $wp_filesystem;
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					$file = substr( $file, 0, 2 ) == '//' ? fabrica_add_protocol( $file ) : str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
					return $wp_filesystem->get_contents( $file );
				} else {
					if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'fabrica' ), $file ) );
					}
				}
			}
		}
		...
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fga' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'fabrica' ), $file ) );
					}
				}
			}
		}
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( ! $wp_filesystem->is_dir( $path ) ) {
						...
						if ( ! wp_mkdir_p( $path ) ) {
							if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
								// Translators: Add the file name to the message
								throw new Exception( sprintf( esc_html__( 'Create a folder "%s" failed', 'fabrica' ), $path ) );
							}
						} else {
							return true;
						}
					...
					if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Create a folder "%s" failed', 'fabrica' ), $path ) );
					}
				}
			}
		}
		...
		if ( ! empty( $path ) ) {
			if ( function_exists( 'trx_addons_unlink' ) ) {
				...
				if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
					...
					if ( fabrica_is_on( fabrica_get_theme_option( 'debug_mode' ) ) ) {
						// Translators: Add the file name to the message
						throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Delete a file/folder "%s" failed', 'fabrica' ), $path ) );
					}
				}
			}
		}
	...
	function fabrica_copy( $src, $dst ) {
		...
		if ( ! empty( $src ) && ! empty( $dst ) ) {
			if ( function_exists( 'trx_addons_copy' ) ) {
				...
				$src = fabrica_prepare_path( $src );
				$dst = fabrica_prepare_path( $dst );
...
if ( ! function_exists( 'fabrica_is_external_url' ) ) {
	...
	function fabrica_is_external_url( $url ) {
		return fabrica_is_url( $url ) && strpos( $url, fabrica_remove_protocol_from_url( apply_filters( 'trx_addons_filter_home_url', get_home_url() ), true ) ) === false;
	}
}
